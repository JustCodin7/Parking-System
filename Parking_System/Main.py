...
def calculate_fee(mall_name, total_hours):
        ...
        for user in users:
            if user["username"] == username and user["password"] == password:
                ...
                if logged_in_user["role"] == "Customer":
                    ...
                    while True:
                        if active_mall:
                            print("--- Customer Menu [Active Mall:", active_mall, "] ---")
                        else:
                            print("--- Customer Menu [No Mall Selected] ---")
                        ...
                        if customer_choice == "1":
                            ...
                            if selected_mall != None:
                                ...
                                if currently_parked >= total_capacity:
                                    ...
                                    plate = input("Enter your vehicle number plate: ").strip()
                                    if plate == "":
                                        print("Invalid plate number. Please try again.")
                                        continue
                            ...
                            found_record = None
                            for record in parking_list:
                                if record["plate"] == plate and record["exit_time"] == None and record["username"] == logged_in_user["username"]:
                                    found_record = record

                            if found_record == None:
                                print("No active parking found for plate", plate, "under your account.")
                            else:
                                ...
                                fee = calculate_fee(found_record["mall"], total_hours)
                                ...
                                for record in parking_list:
                                    if record["mall"] == mall_name:
                                        ...
                                        if record["exit_time"] != None and record["entry_time"] != None:
                                            ...
                                            duration = exit - entry
                                            total_duration = total_duration + int(duration.total_seconds()) // 60
